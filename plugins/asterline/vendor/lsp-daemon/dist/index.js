...
function daemonBaseDir(env = process.env) {
  ...
  const home = asterlineHome && asterlineHome.length > 0 ? asterlineHome : join2(homedir(), ".asterline");
  return join2(home, "asterline-lsp", "daemon");
}
...
function resolveSocketPath(dir, version) {
  ...
  if (natural.length < MAX_SOCKET_PATH_LENGTH)
    return natural;
  return join2(tmpdir(), `asterline-lsp-${version}-${digest}.sock`);
}
...
function getInstallDecisionsPath() {
  ...
  if (!override)
    return join3(homedir2(), ".asterline", "lsp-install-decisions.json");
...
function getProjectConfigPaths() {
  ...
  return [join4(contextCwd(), ".asterline", "lsp-client.json")];
}
function getUserConfigPath() {
  const userOverride = contextEnv("LSP_TOOLS_MCP_USER_CONFIG");
  if (!userOverride)
    return join4(homedir3(), ".asterline", "lsp-client.json");
