...
function makeSessionStartInput(overrides: Partial<AsterlineSessionStartInput> = {}): AsterlineSessionStartInput {
	return {
...
describe("telemetry CLI session-start hook (subprocess)", () => {
	describe("#given ASTERLINE_DISABLE_POSTHOG=1 set in environment", () => {
		it("#when CLI receives valid SessionStart JSON #then exits 0 with no stdout output", async () => {
			...
			const dataDir = mkdtempSync(path.join(tmpdir(), "asterline-telemetry-data-"));
			tempDirectories.push(dataDir);

			const result = await runHookCli(payload, {
				ASTERLINE_DISABLE_POSTHOG: "1",
		...
		it("#when CLI runs from an isolated snapshot without node_modules #then exits 0 with no output", async () => {
			const payload = JSON.stringify(makeSessionStartInput());
			const snapshotRoot = mkdtempSync(path.join(tmpdir(), "asterline-telemetry-snapshot-"));
			const dataDir = mkdtempSync(path.join(tmpdir(), "asterline-telemetry-data-"));
	...
	describe("#given ASTERLINE_SEND_ANONYMOUS_TELEMETRY=0 set in environment", () => {
		it("#when CLI receives valid SessionStart JSON #then exits 0 with no stdout output", async () => {
			const payload = JSON.stringify(makeSessionStartInput());
			const dataDir = mkdtempSync(path.join(tmpdir(), "asterline-telemetry-data-"));
			tempDirectories.push(dataDir);
			...
			const result = await runHookCli(payload, {
				ASTERLINE_SEND_ANONYMOUS_TELEMETRY: "0",
				XDG_DATA_HOME: dataDir,
			});
