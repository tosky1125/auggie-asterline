...
function createDataHomePath(): string {
	const tempPath = mkdtempSync(path.join(tmpdir(), "asterline-telemetry-diagnostics-"));
...
describe("telemetry diagnostics", () => {
	it("#given a telemetry failure #when diagnostics are written #then JSONL is stored under the asterline-runtime data directory", () => {
