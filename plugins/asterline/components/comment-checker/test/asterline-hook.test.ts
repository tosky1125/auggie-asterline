...
import {
	...
	extractAsterlineCommentCheckRequests,
	runCommentCheckerPostToolUse,
} from "../src/asterline-hook.ts";
...
function postToolUseInput(overrides: Partial<AsterlinePostToolUseInput> = {}): AsterlinePostToolUseInput {
	return {
		session_id: "thread-1",
		turn_id: "turn-1",
...
describe("extractAsterlineCommentCheckRequests", () => {
	it("#given asterline apply_patch command #when extracting #then returns edit request for changed file", () => {
		const requests = extractAsterlineCommentCheckRequests(postToolUseInput());
	...
	it("#given apply_patch metadata files #when extracting #then metadata takes priority over raw patch text", () => {
		const requests = extractAsterlineCommentCheckRequests(
			postToolUseInput({
				tool_input: {
					command: [
						"*** Begin Patch",
	...
	it("#given nested apply_patch metadata #when extracting #then result and metadata containers are supported", () => {
		const resultRequests = extractAsterlineCommentCheckRequests(
			postToolUseInput({
				tool_response: {
					result: {
		...
		const metadataRequests = extractAsterlineCommentCheckRequests(
			postToolUseInput({
				tool_response: {
					metadata: {
	...
	it("#given failed tool response #when extracting #then no comment-check request is emitted", () => {
		const textFailureRequests = extractAsterlineCommentCheckRequests(
			postToolUseInput({
				tool_response: "failed to apply patch",
			}),
		);
		const structuredFailureRequests = extractAsterlineCommentCheckRequests(
			postToolUseInput({
	...
	it("#given asterline write payload #when extracting #then returns write request", () => {
		const requests = extractAsterlineCommentCheckRequests(
			postToolUseInput({
				tool_name: "write",
				tool_input: {
	...
	it("#given asterline edit payload #when extracting #then returns edit request", () => {
		const requests = extractAsterlineCommentCheckRequests(
			postToolUseInput({
				tool_name: "edit",
				tool_input: {
	...
	it("#given one-sided asterline edit payload #when extracting #then returns no requests", () => {
		const requests = extractAsterlineCommentCheckRequests(
			postToolUseInput({
				tool_name: "edit",
				tool_input: {
	...
	it("#given asterline multi_edit payload #when extracting #then returns multiedit request", () => {
		const requests = extractAsterlineCommentCheckRequests(
			postToolUseInput({
...
describe("runCommentCheckerPostToolUse", () => {
	...
	it("#given transcript path #when hook runs #then forwards it to checker input", async () => {
		...
		await runCommentCheckerPostToolUse(
			postToolUseInput({
				transcript_path: "/tmp/asterline-comment-checker-transcript.jsonl",
				tool_name: "write",
				tool_input: {
					file_path: "src/example.ts",
					content: "// explains value\nconst value = 1;\n",
		...
		expect(transcriptPath).toBe("/tmp/asterline-comment-checker-transcript.jsonl");
	});
	...
	it("#given Asterline canonical context-window transcript and long checker warning #when hook blocks #then it caps feedback", async () => {
		const root = mkdtempSync(path.join(tmpdir(), "asterline-comment-checker-context-pressure-"));
		tempDirs.push(root);
