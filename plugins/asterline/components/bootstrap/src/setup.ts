...
import { resolveAsterlineInstallerBinDir } from "../../../../scripts/install/bin-dir.mjs";
import { linkCachedPluginBins, linkRootRuntimeBin } from "../../../../scripts/install/bin-links.mjs";
import { updateAsterlineConfig } from "../../../../scripts/install/config.mjs";
import type { AsterlineAgentConfig } from "../../../../scripts/install/config.mjs";
...
export const SETUP_PLUGIN_NAME = "asterline";
...
interface AgentLinkOutcome {
	readonly agentConfigs: readonly AsterlineAgentConfig[];
...
// though Asterline deletes old version dirs on upgrade (core-plugins store.rs).
...
async function linkBundledAgentsStep(options: WorkerSetupOptions): Promise<AgentLinkOutcome> {
	const agentsTarget = join(options.asterlineHome, "agents");
	try {
		...
		// Asterline-managed marketplace cache).
...
async function updateConfigStep(
	...
	inputs: { agentConfigs: readonly AsterlineAgentConfig[]; gitBashEnabled: boolean },
	degraded: BootstrapDegradedEntry[],
): Promise<void> {
	const configPath = join(options.asterlineHome, "config.toml");
	try {
...
// degraded mode there: record the asterline-cli ledger entry and log the same
// warning install-local.mjs prints instead of leaving a broken `asterline` link.
