...
export const INSTALL_SNAPSHOT_FILENAME = "asterline-install.json";

const DEFAULT_MARKETPLACE_NAME = "sisyphuslabs";
const MAX_ASTERLINE_HOME_WALK_UP_LEVELS = 6;
...
export type AsterlineHomeSource = "env" | "walk-up" | "default";

export interface AsterlineHomeResolution {
	readonly path: string;
	readonly source: AsterlineHomeSource;
}

export interface ResolveAsterlineHomeOptions {
	readonly env: Record<string, string | undefined>;
	readonly pluginRoot?: string;
}

export async function resolveAsterlineHome(options: ResolveAsterlineHomeOptions): Promise<AsterlineHomeResolution> {
	...
	return { path: join(homedir(), ".asterline"), source: "default" };
}
