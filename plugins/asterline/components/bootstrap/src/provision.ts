...
export const SG_FORCE_PROVISION_ENV_KEY = "ASTERLINE_BOOTSTRAP_FORCE_PROVISION";
...
export function sgProvisionDestination(context: BootstrapWorkerContext, arch: string): string {
	...
	return join(context.asterlineHome, "runtime", "ast-grep", `${context.platform}-${arch}`, binaryName);
}
