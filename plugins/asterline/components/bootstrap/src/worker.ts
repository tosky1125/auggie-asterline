...
import { bootstrapLocks, resolveBootstrapStatePath, resolveAsterlineHome } from "./environment.ts";
import { runSgProvision } from "./provision.ts";
import type { SgProvisionSeams } from "./provision.ts";
import { runWorkerSetup } from "./setup.ts";

export const BOOTSTRAP_DOCTOR_HINT = "npx asterline-ai doctor";
...
export function parseWorkerFlags(argv: readonly string[]): BootstrapWorkerFlags {
	let asterlineHome: string | undefined;
	...
	return {
		once,
		...(asterlineHome === undefined ? {} : { asterlineHome }),
...
export function resolvePluginDataRoot(env: Record<string, string | undefined>): string {
	...
	return join(homedir(), ".local", "share", "asterline");
}

export async function readPluginVersion(pluginRoot: string): Promise<string | undefined> {
	try {
		const parsed: unknown = JSON.parse(await readFile(join(pluginRoot, ".augment-plugin", "plugin.json"), "utf8"));
...
export async function runBootstrapWorker(options: RunBootstrapWorkerOptions = {}): Promise<BootstrapWorkerResult> {
	...
	try {
		...
		const context: BootstrapWorkerContext = { asterlineHome, env, flags, now, platform, pluginData, pluginRoot, pluginVersion };
		...
		if (pluginVersion === undefined) {
			degraded.push({
				...
				reason: `plugin version unresolved from ${join(pluginRoot, ".augment-plugin", "plugin.json")}`,
			});
		}
