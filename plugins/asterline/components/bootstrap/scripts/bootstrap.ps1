# Asterline bootstrap SessionStart hook for native Windows (Windows PowerShell 5.1).
# Asterline runs hooks through %COMSPEC%, so node may be absent from PATH entirely;
...
# (sha256-verified, extracted under <asterlineHome>\runtime\node, USER PATH append).
...
$script:LogPath = $null
$script:DoctorHint = "npx asterline-ai doctor"
...
function Resolve-AsterlineHome {
	if (-not [string]::IsNullOrWhiteSpace($env:ASTERLINE_HOME)) { return $env:ASTERLINE_HOME.Trim() }
	...
	return (Join-Path $env:USERPROFILE ".asterline")
}
...
function Get-PortableNodeDirectory {
	param([string]$AsterlineHome, $Manifest)
	$runtimeRoot = Join-Path $AsterlineHome "runtime\node"
...
function Resolve-NodeCommand {
	...
	if ($null -ne $Manifest) {
		$candidateDirectories += (Get-PortableNodeDirectory -AsterlineHome $AsterlineHome -Manifest $Manifest)
	}
...
function Add-UserPathEntry {
	...
	Write-BootstrapLog ("appended " + $Directory + " to the USER PATH; restart Asterline so new sessions can see node")
}
...
function Install-PortableNode {
	...
	$zipPath = Join-Path $env:TEMP ("asterline-node-v" + $Manifest.version + "-win-x64.zip")
	...
	$nodeDirectory = Get-PortableNodeDirectory -AsterlineHome $AsterlineHome -Manifest $Manifest
...
function Resolve-GitBash {
	if (-not [string]::IsNullOrWhiteSpace($env:ASTERLINE_GIT_BASH_PATH)) {
		$envCandidate = $env:ASTERLINE_GIT_BASH_PATH.Trim()
...
function Initialize-GitBash {
	...
	if (($null -eq $resolution) -and ($env:ASTERLINE_SKIP_GIT_BASH_AUTO_INSTALL -ne "1")) {
		$winget = Get-Command winget -ErrorAction SilentlyContinue
		if ($null -ne $winget) {
	...
	if ($resolution.Source -eq "path") {
		$persisted = [Environment]::GetEnvironmentVariable("ASTERLINE_GIT_BASH_PATH", "User")
		if ([string]::IsNullOrWhiteSpace($persisted)) {
			[Environment]::SetEnvironmentVariable("ASTERLINE_GIT_BASH_PATH", $resolution.Path, "User")
			Write-BootstrapLog ("persisted ASTERLINE_GIT_BASH_PATH=" + $resolution.Path + " in the USER environment; restart Asterline to pick it up")
		}
	}
}
...
function Write-ProvisioningIncompleteNotice {
	$notice = "Asterline bootstrap: Node.js is not available yet. Install Node LTS (winget install OpenJS.NodeJS.LTS), then restart Asterline. Diagnose with: " + $script:DoctorHint
	$payload = @{
		hookSpecificOutput = @{
...
function Invoke-Bootstrap {
	...
	$asterlineHome = Resolve-AsterlineHome
	Write-BootstrapLog ("asterline home resolved to " + $asterlineHome)
	$manifest = Get-NodeManifest
	$nodeExe = Resolve-NodeCommand -AsterlineHome $asterlineHome -Manifest $manifest
	if ($null -ne $nodeExe) {
	...
	$nodeExe = Resolve-NodeCommand -AsterlineHome $asterlineHome -Manifest $manifest
	if ($null -eq $nodeExe) {
		$nodeExe = Install-PortableNode -AsterlineHome $asterlineHome -Manifest $manifest
	}
	Initialize-GitBash
	if ($null -eq $nodeExe) {
