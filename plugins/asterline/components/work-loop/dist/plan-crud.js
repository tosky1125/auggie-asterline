...
import { workLoopBriefPath, workLoopBriefRelativePath, workLoopDir, workLoopGoalsPath, workLoopGoalsRelativePath, workLoopLedgerPath, workLoopLedgerRelativePath } from "./paths.js";
...
function assertNonEmpty(value, label) { const trimmed = value?.trim(); if (!trimmed)
    throw new WorkLoopError(`Missing ${label}.`, "WORK_LOOP_ARGUMENT_MISSING"); return trimmed; }
...
export async function createWorkLoopPlan(repoRoot, args, scope) {
    return withWorkLoopMutationLock(repoRoot, scope, async () => {
        ...
        const plan = { version: 1, createdAt: now, updatedAt: now, briefPath: workLoopBriefRelativePath(scope), goalsPath: workLoopGoalsRelativePath(scope), ledgerPath: workLoopLedgerRelativePath(scope), hostGoalMode: args.hostGoalMode ?? "aggregate", goals };
        if (plan.hostGoalMode === "aggregate")
            plan.asterlineObjective = aggregateAsterlineObjectiveForScope(scope);
...
function completedPlanExistsError(scope) {
    ...
    ].join(" "), "WORK_LOOP_PLAN_EXISTS_COMPLETE");
}
export async function addWorkLoopGoal(repoRoot, args, scope) {
    return withWorkLoopMutationLock(repoRoot, scope, async () => {
...
export async function startNextWorkLoop(repoRoot, args = {}, scope) {
    return withWorkLoopMutationLock(repoRoot, scope, async () => {
        const plan = await readWorkLoopPlan(repoRoot, scope);
        const now = iso();
...
export function summarizeWorkLoopPlan(plan) {
