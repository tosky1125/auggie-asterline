...
import { workLoopDir } from "../src/paths.js";
import { readWorkLoopPlan, writePlan } from "../src/plan-io.js";
...
async function readLastLedgerEntry(repo: string): Promise<WorkLoopLedgerEntry> {
	const lines = (await readFile(join(repo, ".asterline/work-loop/ledger.jsonl"), "utf8")).trim().split("\n");
	const last = lines.at(-1);
	if (last === undefined) throw new Error("expected ledger entry");
	return JSON.parse(last);
}

function firstGoal(plan: WorkLoopPlan): WorkLoopItem {
...
function makePlan(overrides: Partial<WorkLoopPlan> = {}): WorkLoopPlan {
	return {
		...
		asterlineObjective: "Complete the durable work-loop plan in .asterline/work-loop/goals.json",
...
describe("recordEvidence (status=pass)", () => {
	...
	it("persists the change so a fresh read sees status=pass", async () => {
		...
		const criterion = firstGoal(await readWorkLoopPlan(repo)).successCriteria.find((c) => c.id === "C001");
...
describe("recordEvidence error cases", () => {
	it("throws when goalId not found", async () => {
		...
		).rejects.toBeInstanceOf(WorkLoopError);
	});
