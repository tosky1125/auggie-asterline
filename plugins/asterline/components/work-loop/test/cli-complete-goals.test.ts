...
import { workLoopLedgerPath } from "../src/paths.ts";
...
async function ledgerKinds(): Promise<string[]> {
	const raw = await readFile(workLoopLedgerPath(testDir), "utf8");
	return raw
		.split(/\r?\n/)
		.filter(Boolean)
...
async function createPlan(): Promise<void> {
	expect(await workLoopCommand(["create-goals", "--brief", "- Goal A\n- Goal B", "--json"])).toBe(0);
	resetOutput();
}

describe("workLoopCommand complete-goals", () => {
	it("#given a pending plan #when complete-goals starts the next goal #then returns a Asterline create_goal instruction", async () => {
		await createPlan();

		expect(await workLoopCommand(["complete-goals", "--json"])).toBe(0);
