...
import { normalizeHostGoalMode, printStatus, WORK_LOOP_HELP } from "../src/cli-output.js";
...
describe("WORK_LOOP_HELP", () => {
	it("mentions the installed bundle + every subcommand", () => {
		...
		expect(WORK_LOOP_HELP).toContain("checkpoint");
		expect(WORK_LOOP_HELP).toContain("steer");
		expect(WORK_LOOP_HELP).toContain("record-evidence");
		expect(WORK_LOOP_HELP).toContain("criteria");
		expect(WORK_LOOP_HELP).toContain("add-goal");
		expect(WORK_LOOP_HELP).toContain("record-review-blockers");
	});

	it("never mentions the legacy typo", () => {
		const typo = ["o", "m", "x"].join("");

		expect(WORK_LOOP_HELP).not.toMatch(new RegExp(typo, "i"));
	});
});
...
describe("normalizeHostGoalMode", () => {
	it("returns aggregate when undefined", () => {
		expect(normalizeHostGoalMode(undefined)).toBe("aggregate");
	});

	it("returns the explicit value when valid", () => {
		expect(normalizeHostGoalMode("per_story")).toBe("per_story");
	});

	it("throws WorkLoopError when invalid", () => {
		expect(() => normalizeHostGoalMode("per-story")).toThrow(WorkLoopError);
	});
});
