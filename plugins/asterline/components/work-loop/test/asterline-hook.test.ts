...
import {
	...
	applyUserPromptWorkLoopSteering,
...
import type { WorkLoopPlan } from "../src/types.js";
...
async function bootstrapPlanRepo(): Promise<string> {
	...
	await mkdir(workLoopDir(repoRoot, { sessionId: DEFAULT_SESSION_ID }), { recursive: true });
	await writePlan(repoRoot, samplePlan(), { sessionId: DEFAULT_SESSION_ID });
	return repoRoot;
}

function samplePlan(): WorkLoopPlan {
	return {
		version: 1,
		createdAt: NOW,
		updatedAt: NOW,
		...
		goals: [
			{
				id: "G001",
				title: "Build hook",
				objective: "Apply safe steering directives from Asterline hooks.",
				status: "pending",
				successCriteria: [],
				attempt: 0,
				createdAt: NOW,
...
function payloadWithRuntimeEvent(hookEventName: string): UserPromptSubmitPayload {
	const input = payload(
		'ASTERLINE_WORK_LOOP_STEER: {"kind":"annotate_ledger","source":"user_prompt_submit","evidence":"x","rationale":"y"}',
		"/tmp",
	);
...
describe("parseUserPromptSubmitPayload", () => {
	it("parses valid JSON payload", async () => {
		...
		expect(parsed?.prompt).toContain("ASTERLINE_WORK_LOOP_STEER");
	});
...
describe("applyUserPromptWorkLoopSteering - ASTERLINE directive patterns", () => {
	it("processes ASTERLINE_WORK_LOOP_STEER: prompt and returns audit text on success", async () => {
		const repoRoot = await bootstrapPlanRepo();
		const out = await applyUserPromptWorkLoopSteering(
			payload(
				'ASTERLINE_WORK_LOOP_STEER: {"kind":"annotate_ledger","source":"user_prompt_submit","evidence":"x","rationale":"y"}',
				repoRoot,
			),
	...
	it("#given a Asterline session id #when steering from a hook #then writes the session-scoped ledger", async () => {
		const repoRoot = await bootstrapPlanRepo();
		...
		expect(await readFile(workLoopLedgerPath(repoRoot, { sessionId: DEFAULT_SESSION_ID }), "utf8")).toContain(
			"steering_accepted",
		);
	});

	it("processes asterline.work-loop.steer: pattern", async () => {
		const repoRoot = await bootstrapPlanRepo();
		const out = await applyUserPromptWorkLoopSteering(
			payload(
				'asterline.work-loop.steer: {"kind":"annotate_ledger","source":"user_prompt_submit","evidence":"x","rationale":"y"}',
				repoRoot,
			),
		);
		expect(out).toContain("accepted");
...
describe("applyUserPromptWorkLoopSteering - non-matching prompts", () => {
	it("returns empty string when no directive in prompt", async () => {
		expect(await applyUserPromptWorkLoopSteering(payload("just a normal user message", "/tmp"))).toBe("");
	});

	it("returns empty when hook_event_name is not UserPromptSubmit", async () => {
		expect(await applyUserPromptWorkLoopSteering(payloadWithRuntimeEvent("PostToolUse"))).toBe("");
	});
});

describe("applyUserPromptWorkLoopSteering - error swallowing", () => {
	...
	it("returns empty when steering proposal is malformed JSON after marker", async () => {
		const out = await applyUserPromptWorkLoopSteering(payload("ASTERLINE_WORK_LOOP_STEER: {bad", "/tmp"));
		expect(out).toBe("");
	});
});

describe("runWorkLoopHookCli (stdin/stdout integration)", () => {
	it("reads stdin, applies steering, writes audit to stdout", async () => {
		...
		await runWorkLoopHookCli(stdin, capture.stdout);
		expect(capture.read().length).toBeGreaterThan(0);
	});

	it("writes nothing when stdin is empty", async () => {
		const capture = captureStdout();
		await runWorkLoopHookCli(Readable.from([""]), capture.stdout);
...
describe("runPreToolUseGoalBudgetGuardCli", () => {
	it("#given Asterline PreToolUse stdin with budgeted create_goal #when CLI hook runs #then it writes blocking JSON", async () => {
		// given
		const stdin = Readable.from([
