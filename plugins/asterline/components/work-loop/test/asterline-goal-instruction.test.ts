...
import { buildHostGoalInstruction } from "../src/host-goal-instruction.js";
...
describe("buildHostGoalInstruction aggregate mode", () => {
	it("references the aggregate handoff and the .asterline/work-loop/goals.json artifact", () => {
		const { text } = buildHostGoalInstruction({ plan: makePlan({ hostGoalMode: "aggregate" }), goal: makeGoal() });
		expect(text).toContain("aggregate");
		expect(text).toContain(".asterline/work-loop/goals.json");
	});

	it("given aggregate mode when rendering create_goal payload then omits numeric limits", () => {
		const { json, text } = buildHostGoalInstruction({
			plan: makePlan({ hostGoalMode: "aggregate" }),
			goal: makeGoal(),
		});
		expect(json).toEqual({
			objective: WORK_LOOP_AGGREGATE_ASTERLINE_OBJECTIVE,
		});
	...
	it("#given a scoped plan #when rendering final commands #then includes the session id option", () => {
		const { text } = buildHostGoalInstruction({
			plan: makePlan({
				...
				goalsPath: ".asterline/work-loop/session-A/goals.json",
				ledgerPath: ".asterline/work-loop/session-A/ledger.jsonl",
			}),
			goal: makeGoal(),
			isFinal: true,
		});
...
describe("buildHostGoalInstruction per_story mode", () => {
	it("uses the goal's own objective for create_goal", () => {
		const goal = makeGoal({ objective: "Build the auth service" });
		const { text } = buildHostGoalInstruction({ plan: makePlan({ hostGoalMode: "per_story" }), goal });
		expect(text).toContain("Build the auth service");
	});
});

describe("buildHostGoalInstruction criteria section", () => {
	it("lists every successCriteria entry with id + scenario + status", () => {
		...
		const { text } = buildHostGoalInstruction({ plan: makePlan(), goal });
...
describe("buildHostGoalInstruction rebrand audit", () => {
	it("emits no legacy brand references in any rendered string", () => {
		const legacyBrand = ["o", "m", "x"].join("");
		const { text } = buildHostGoalInstruction({ plan: makePlan(), goal: makeGoal() });
		expect(text).not.toMatch(new RegExp(legacyBrand, "i"));
	});

	it("references .asterline/work-loop in artifact paths", () => {
		const { text } = buildHostGoalInstruction({ plan: makePlan({ hostGoalMode: "aggregate" }), goal: makeGoal() });
		expect(text).toContain(".asterline/work-loop");
	});
});
