...
import type { SteerWorkLoopResult, WorkLoopPlan } from "../src/types.js";
...
function steerResult(overrides: Partial<SteerWorkLoopResult> = {}): SteerWorkLoopResult {
	return {
		plan: plan(),
		accepted: true,
		audit: {
...
describe("parseSteeringKind", () => {
	...
	for (const testCase of [
		...
		it(testCase.name, () => {
			expect(() => parseSteeringKind(testCase.args)).toThrow(WorkLoopError);
		});
	}
});
...
describe("parseSteeringProposal revise_criterion", () => {
	...
	for (const testCase of [
		...
		it(testCase.name, async () => {
			await expect(parseSteeringProposal(testCase.args)).rejects.toThrow(WorkLoopError);
		});
	}
});
...
describe("printSteerResult", () => {
	...
	it("prints human-readable when json=false", () => {
		...
		expect(output).toContain("work-loop steer: accepted add_subgoal");
		expect(output).toContain("work-loop status");
	});
});
