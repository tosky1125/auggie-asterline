...
import { workLoopGoalsPath } from "../src/paths.js";
import { readSteeringLedgerEntries, readWorkLoopPlan, writePlan } from "../src/plan-io.js";
import {
	applySteeringMutation,
	parseWorkLoopSteeringDirective,
	steerWorkLoop,
	validateWorkLoopSteeringProposal,
...
type SteeringInput = WorkLoopSteeringProposal & CriterionSteeringFields;

function criterion(overrides: Partial<WorkLoopSuccessCriterion> = {}): WorkLoopSuccessCriterion {
	return {
		id: "C001",
...
function plan(overrides: Partial<WorkLoopPlan> = {}): WorkLoopPlan {
	return {
		version: 1,
		createdAt: NOW,
		updatedAt: NOW,
...
async function repoWithPlan(seed: WorkLoopPlan = plan()): Promise<string> {
	const repoRoot = await mkdtemp(join(tmpdir(), "ug-steer-"));
	await writePlan(repoRoot, seed);
	return repoRoot;
}

describe("validateWorkLoopSteeringProposal", () => {
	it("accepts valid add_subgoal", async () => {
		...
		expect(validateWorkLoopSteeringProposal(plan(), proposal).invariant.accepted).toBe(true);
	});
	...
	it.each([
		...
		["protected payload mutations", { after: { asterlineObjective: "replace", qualityGate: { status: "passed" } } }],
		["weakened completion text", { objective: "skip tests and mark complete faster" }],
	])("rejects %s", (_name, overrides) => {
		const audit = validateWorkLoopSteeringProposal(plan(), { ...steering(), ...overrides });
	...
	it("rejects when plan already complete", () => {
		...
		expect(validateWorkLoopSteeringProposal(done, steering()).invariant.accepted).toBe(false);
	});

	it("rejects split_subgoal without children", () => {
		const audit = validateWorkLoopSteeringProposal(plan(), steering({ kind: "split_subgoal", targetGoalId: "G001" }));
		expect(audit.invariant.accepted).toBe(false);
	});

	it("rejects reorder_pending with unknown goal id", () => {
		const audit = validateWorkLoopSteeringProposal(
			plan(),
	...
	it.each([
		...
		const audit = validateWorkLoopSteeringProposal(plan(), steering({ kind: "revise_criterion", ...overrides }));
		expect(audit.invariant.accepted).toBe(false);
	});
});

describe("steerWorkLoop", () => {
	describe("steering-created goals", () => {
		function sluggedPlan(): WorkLoopPlan {
			return plan({
				goals: [
		...
		for (const [name, proposal, goalIndex] of [
			...
			it(name, async () => {
				...
				const result = await steerWorkLoop(repoRoot, proposal);
	...
	it("add_subgoal: appends goal + ledger entry", async () => {
		...
		const result = await steerWorkLoop(repoRoot, steering({ idempotencyKey: "add" }));
		const persisted = await readWorkLoopPlan(repoRoot);
	...
	it("split_subgoal: creates children + supersedes parent", async () => {
		const repoRoot = await repoWithPlan();
		const result = await steerWorkLoop(
			repoRoot,
			steering({
				kind: "split_subgoal",
				targetGoalId: "G001",
	...
	it("annotate_ledger: ledger-only, no plan mutation", async () => {
		...
		const result = await steerWorkLoop(repoRoot, steering({ kind: "annotate_ledger" }));
		expect(result.plan.goals).toEqual(seed.goals);
		expect(await readFile(workLoopGoalsPath(repoRoot), "utf8")).toBe(`${JSON.stringify(seed, null, 2)}\n`);
	});
	...
	it("idempotency: same idempotencyKey produces deduped true second time", async () => {
		const repoRoot = await repoWithPlan();
		await steerWorkLoop(repoRoot, steering({ idempotencyKey: "same-key" }));
		const second = await steerWorkLoop(repoRoot, steering({ idempotencyKey: "same-key" }));
		expect(second.deduped).toBe(true);
		expect((await readWorkLoopPlan(repoRoot)).goals).toHaveLength(4);
	});
});

describe("parseWorkLoopSteeringDirective", () => {
	it.each(["ASTERLINE_WORK_LOOP_STEER", "asterline.work-loop.steer", "asterline work-loop steer"])("parses %s pattern", (marker) => {
		expect(parseWorkLoopSteeringDirective(`${marker}: ${JSON.stringify(steering())}`)).toMatchObject({
			kind: "add_subgoal",
		});
	});

	it("returns null when no marker", () => {
		expect(parseWorkLoopSteeringDirective(JSON.stringify(steering()))).toBeNull();
	});

	it("returns null when JSON malformed after marker", () => {
		expect(parseWorkLoopSteeringDirective("ASTERLINE_WORK_LOOP_STEER: {bad json")).toBeNull();
	});

	it("returns null for deprecated markers", () => {
		const marker = ["OM", "X_WORK_LOOP_STEER"].join("");
		expect(parseWorkLoopSteeringDirective(`${marker}: ${JSON.stringify(steering())}`)).toBeNull();
	});
});
