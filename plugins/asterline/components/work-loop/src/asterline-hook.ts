...
import { parseWorkLoopSteeringDirective, steerWorkLoop } from "./steering.js";
...
const CREATE_GOAL_TOOL_NAME = ["create", "goal"].join("_");
const CREATE_GOAL_PAYLOAD_WARNING =
	"Use native goal activation with objective only. Omit token_budget so the goal stays unlimited, and put lifecycle status changes on update_goal.";
...
export async function applyUserPromptWorkLoopSteering(payload: UserPromptSubmitPayload): Promise<string> {
	try {
		if (payload.hook_event_name !== "UserPromptSubmit") return "";
		const proposal = parseWorkLoopSteeringDirective(payload.prompt);
		if (proposal === null) return "";
		const result = await steerWorkLoop(payload.cwd, proposal, payloadScope(payload));
...
function payloadScope(payload: UserPromptSubmitPayload): WorkLoopScope {
...
export async function runWorkLoopHookCli(stdin: NodeJS.ReadableStream, stdout: NodeJS.WritableStream): Promise<void> {
	try {
		const payload = parseUserPromptSubmitPayload(await readAll(stdin));
		if (payload === null) return;
		const output = await applyUserPromptWorkLoopSteering(payload);
