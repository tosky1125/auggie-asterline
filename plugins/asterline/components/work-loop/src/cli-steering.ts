...
import type { SteerWorkLoopResult, WorkLoopSteeringChildGoal, WorkLoopSteeringMutationKind, WorkLoopSteeringProposal, WorkLoopSteeringSource, WorkLoopSuccessCriterionUserModel } from "./types.js";
...
export type CliSteeringProposal = WorkLoopSteeringProposal & { readonly goalId?: string; readonly scenario?: string; readonly expectedEvidence?: string; readonly userModel?: WorkLoopSuccessCriterionUserModel };

function isKind(value: string | undefined): value is WorkLoopSteeringMutationKind { return value !== undefined && WORK_LOOP_STEERING_MUTATION_KINDS.some((kind) => kind === value); }
function isSource(value: string | undefined): value is WorkLoopSteeringSource { return value !== undefined && SOURCES.some((source) => source === value); }
function isModel(value: string): value is WorkLoopSuccessCriterionUserModel { return WORK_LOOP_SUCCESS_CRITERION_USER_MODELS.some((model) => model === value); }
function fail(message: string, code: string, details: Record<string, unknown>): never { throw new WorkLoopError(message, code, { details }); }
function text(value: string | undefined, field: string): string | undefined { if (value === undefined) return undefined; const trimmed = value.trim(); if (trimmed.length > 0) return trimmed; return fail(`Empty ${field}.`, "WORK_LOOP_STEERING_FIELD_EMPTY", { field }); }
function required(argv: readonly string[], flag: string): string { const value = text(readValue(argv, flag), flag); return value ?? fail(`Missing ${flag}.`, "WORK_LOOP_STEERING_FIELD_REQUIRED", { flag }); }
function requiredGoal(argv: readonly string[]): string { const value = text(parseGoalArg(argv), "--goal-id"); return value ?? fail("Missing --goal-id.", "WORK_LOOP_GOAL_ID_REQUIRED", { flag: "--goal-id" }); }
...
export function parseSteeringKind(argv: readonly string[]): WorkLoopSteeringMutationKind {
...
export function parseSteeringSource(argv: readonly string[]): WorkLoopSteeringSource {
...
async function children(argv: readonly string[], flag: string, needed: boolean): Promise<WorkLoopSteeringChildGoal[]> {
	const input = needed ? required(argv, flag) : text(readValue(argv, flag), flag);
	if (input === undefined) return [];
	const raw = await readJsonInput(input);
	if (!Array.isArray(raw)) return fail(`${flag} must be a JSON array.`, "WORK_LOOP_STEERING_JSON_ARRAY_REQUIRED", { flag });
	const parsed: WorkLoopSteeringChildGoal[] = [];
	for (const item of raw) { const next = child(item); if (next === null) return fail(`${flag} entries require title/objective.`, "WORK_LOOP_STEERING_CHILD_INVALID", { flag }); parsed.push(next); }
	return parsed;
}
...
async function stringArray(argv: readonly string[], flag: string): Promise<string[]> {
	...
	if (!Array.isArray(raw)) return fail(`${flag} must be a JSON array.`, "WORK_LOOP_STEERING_JSON_ARRAY_REQUIRED", { flag });
	const values: string[] = [];
	for (const item of raw) { if (typeof item !== "string") return fail(`${flag} entries must be strings.`, "WORK_LOOP_STEERING_STRING_ARRAY_REQUIRED", { flag }); values.push(text(item, flag) ?? ""); }
	return values;
}

function model(value: string | undefined): WorkLoopSuccessCriterionUserModel | undefined { const trimmed = text(value, "--user-model"); if (trimmed === undefined) return undefined; return isModel(trimmed) ? trimmed : fail(`Invalid --user-model: ${trimmed}.`, "WORK_LOOP_STEERING_USER_MODEL_INVALID", { value: trimmed, expected: WORK_LOOP_SUCCESS_CRITERION_USER_MODELS }); }
...
export async function parseSteeringProposal(argv: readonly string[]): Promise<CliSteeringProposal> {
	...
	switch (kind) {
		...
		case "revise_pending_wording": { const goalId = requiredGoal(argv); const revisedTitle = readValue(argv, "--title"); const revisedObjective = readValue(argv, "--objective"); if (revisedTitle === undefined && revisedObjective === undefined) return fail("revise_pending_wording requires --title or --objective.", "WORK_LOOP_STEERING_UPDATE_REQUIRED", { kind }); return normalizeSteeringProposal({ ...base, goalId, targetGoalId: goalId, ...(revisedTitle === undefined ? {} : { revisedTitle }), ...(revisedObjective === undefined ? {} : { revisedObjective }) }); }
		case "revise_criterion": { const goalId = requiredGoal(argv); const criterionId = required(argv, "--criterion-id"); const scenario = readValue(argv, "--scenario"); const expectedEvidence = readValue(argv, "--expected-evidence"); const userModel = model(readValue(argv, "--user-model")); if (scenario === undefined && expectedEvidence === undefined && userModel === undefined) return fail("revise_criterion requires scenario, expected-evidence, or user-model.", "WORK_LOOP_STEERING_UPDATE_REQUIRED", { kind }); return normalizeSteeringProposal({ ...base, goalId, targetGoalId: goalId, criterionId, ...(scenario === undefined ? {} : { scenario }), ...(expectedEvidence === undefined ? {} : { expectedEvidence }), ...(userModel === undefined ? {} : { userModel }) }); }
...
function normalizedChildren(values: readonly WorkLoopSteeringChildGoal[] | undefined): WorkLoopSteeringChildGoal[] | undefined { if (values === undefined) return undefined; return values.map((item) => ({ title: text(item.title, "child.title") ?? "", objective: text(item.objective, "child.objective") ?? "" })); }
...
export function printSteerResult(result: SteerWorkLoopResult, json: boolean): void {
