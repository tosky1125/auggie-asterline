...
export async function readWorkLoopPlan(repoRoot: string, scope?: WorkLoopScope): Promise<WorkLoopPlan> {
	...
	try {
		...
		throw new WorkLoopError(
			`No work-loop plan found at ${repoRelative(path, repoRoot)}. Run \`asterline work-loop create-goals ...\` first.`,
			"WORK_LOOP_PLAN_MISSING",
			{ cause: error },
		);
	}
	const parsed: WorkLoopPlan = JSON.parse(raw);
...
export async function readSteeringLedgerEntries(repoRoot: string, scope?: WorkLoopScope): Promise<WorkLoopLedgerEntry[]> {
	...
	for (const line of raw.split(/\r?\n/).filter(Boolean)) {
		const entry: WorkLoopLedgerEntry = JSON.parse(line);
