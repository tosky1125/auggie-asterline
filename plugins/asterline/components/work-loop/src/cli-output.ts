...
export function blockedDecisionHandoff(plan: WorkLoopPlan): string {
...
export function normalizeHostGoalMode(value: string | undefined): WorkLoopHostGoalMode {
	if (value === undefined) return "aggregate";
	if (value === "aggregate" || value === "per_story") return value;
	throw new WorkLoopError(
		"Invalid --host-goal-mode; expected aggregate or per_story.",
		"WORK_LOOP_ASTERLINE_GOAL_MODE_INVALID",
		{ details: { value } },
	);
}
