...
export function dynamicDedupKey(rulePath, contentHash) {
    return `${rulePath}::${contentHash}`;
}
export function markStaticInjected(state, rule) {
...
export function markDynamicInjected(state, rule) {
...
export function isStaticInjected(state, rule) {
    return state.staticDedup.has(staticDedupKey(state.cwd ?? "", rule.realPath, rule.contentHash));
}
export function isDynamicInjected(state, rule) {
