...
export function configFromEnvironment(env: NodeJS.ProcessEnv = process.env): PiRulesConfig {
	...
	config.postCompactMaxRuleChars =
		parsePositiveInteger(
			firstEnv(env, "ASTERLINE_RULES_POST_COMPACT_MAX_RULE_CHARS", "PI_RULES_POST_COMPACT_MAX_RULE_CHARS"),
		) ?? config.postCompactMaxRuleChars;
	config.postCompactMaxResultChars =
		parsePositiveInteger(
			firstEnv(env, "ASTERLINE_RULES_POST_COMPACT_MAX_RESULT_CHARS", "PI_RULES_POST_COMPACT_MAX_RESULT_CHARS"),
		) ?? config.postCompactMaxResultChars;
	...
	config.dynamicMaxResultChars =
		parsePositiveInteger(
			firstEnv(env, "ASTERLINE_RULES_DYNAMIC_MAX_RESULT_CHARS", "PI_RULES_DYNAMIC_MAX_RESULT_CHARS"),
		) ?? config.dynamicMaxResultChars;
	...
	config.enabledSources = parseEnabledSources(
		firstEnv(env, "ASTERLINE_RULES_ENABLED_SOURCES", "PI_RULES_ENABLED_SOURCES"),
		disableBundledRules,
	);
	return config;
}
