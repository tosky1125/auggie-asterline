...
asterline-runtime auto-runs LSP diagnostics after every edit and injects the result: any reported error is blocking until resolved. You may also invoke diagnostics explicitly.

# Subagents

asterline-runtime bundles read-only Asterline subagent roles in `ASTERLINE_HOME/agents/`. Spawn: `multi_agent_v1.spawn_agent({"message":"TASK: act as a <role>. ...","fork_context":false})`
...
- `asterline-work-reviewer` - rigorous verification of a finished change
...
- **TUI / CLI / shell binary** - launch through Asterline shell: send input, run the happy path, try one bad input, hit `--help`, read the rendered output.
