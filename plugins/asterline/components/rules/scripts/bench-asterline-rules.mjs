...
import { runPostToolUseHook } from "../dist/asterline-hook.js";
...
async function measureHookFastPathRun() {
	const projectRoot = mkdtempSync(join(tmpdir(), "asterline-rules-hook-bench-"));
	const pluginData = mkdtempSync(join(tmpdir(), "asterline-rules-hook-data-"));
	try {
		mkdirSync(join(projectRoot, "src"), { recursive: true });
		mkdirSync(join(projectRoot, ".asterline", "rules"), { recursive: true });
		...
		for (let index = 0; index < RULE_COUNT; index += 1) {
			writeFileSync(join(projectRoot, ".asterline", "rules", `rule-${index}.md`), ruleContent(`rule-${index}`));
		}
...
function measureRun(targetPaths) {
	const projectRoot = mkdtempSync(join(tmpdir(), "asterline-rules-bench-"));
	try {
...
function makeCandidates(projectRoot) {
	return Array.from({ length: RULE_COUNT }, (_, index) => ({
		path: join(projectRoot, ".asterline", "rules", `rule-${index}.md`),
		realPath: join(projectRoot, ".asterline", "rules", `rule-${index}.md`),
		source: ".asterline/rules",
		distance: 0,
		isGlobal: false,
		isSingleFile: false,
		relativePath: `.asterline/rules/rule-${index}.md`,
	}));
}
