...
import { type AsterlinePostToolUseLike, extractAsterlineToolPaths } from "../src/tool-paths.js";
...
function makeProject(): string {
	const root = mkdtempSync(path.join(tmpdir(), "asterline-rules-paths-"));
...
function postToolUse(input: { toolName: string; toolInput?: unknown; toolResponse?: unknown }): AsterlinePostToolUseLike {
	return {
...
describe("extractAsterlineToolPaths", () => {
	for (const [name, toolName, toolInput] of [
		...
		it(name, () => {
			...
			// when
			const paths = extractAsterlineToolPaths(postToolUse({ toolName, toolInput }), root);
	...
	it("#given apply_patch payload #when extracting #then returns patched file paths", () => {
		...
		// when
		const paths = extractAsterlineToolPaths(
			postToolUse({
