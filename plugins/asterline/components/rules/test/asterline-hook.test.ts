...
const CLAUDE_AND_RULES_ENV = {
	ASTERLINE_RULES_ENABLED_SOURCES: "CLAUDE.md,.asterline/rules",
};
...
function makeTempProject(): { root: string; pluginData: string } {
	const root = mkdtempSync(path.join(tmpdir(), "asterline-rules-project-"));
	const pluginData = mkdtempSync(path.join(tmpdir(), "asterline-rules-data-"));
...
describe("asterline rules hooks", () => {
	...
	it("#given default auto sources #when SessionStart runs #then native Asterline AGENTS.md is not duplicated", async () => {
		// given
		const { root, pluginData } = makeTempProject();
		...
		const parsed = parseHookOutput(output);
		expect(parsed.hookSpecificOutput?.additionalContext).toContain("## Project Instructions");
		expect(parsed.hookSpecificOutput?.additionalContext).not.toContain("Project AGENTS.md should stay Asterline-native.");
		expect(parsed.hookSpecificOutput?.additionalContext).not.toContain(
			"Project CLAUDE.md should stay outside rules hook context.",
		);
	});

	it("#given project AGENTS.md #when SessionStart runs #then rules hook leaves AGENTS.md to Asterline native handling", async () => {
		// given
		const { root, pluginData } = makeTempProject();
	...
	it("#given default auto sources #when excluded AGENTS.md changes #then PostToolUse fingerprint stays stable", async () => {
		...
		const cachedState = readSessionCache(pluginData);
		writeFileSync(path.join(root, "AGENTS.md"), "Native Asterline instructions changed outside asterline-rules auto.");
