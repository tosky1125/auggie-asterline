...
describe("asterline rules PostCompact deduplication", () => {
...
function makeTempProject(): { root: string; pluginData: string } {
	const root = mkdtempSync(path.join(tmpdir(), "asterline-rules-compact-dedup-project-"));
	const pluginData = mkdtempSync(path.join(tmpdir(), "asterline-rules-compact-dedup-data-"));
