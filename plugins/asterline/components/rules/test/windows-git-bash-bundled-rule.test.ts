...
import { type AsterlineSessionStartInput, runSessionStartHook } from "../src/asterline-hook.js";
import { findPluginBundledCandidates } from "../src/rules/finder.js";

const WINDOWS_RULE_DESCRIPTION = "Windows Git Bash guidance for Asterline";
const WINDOWS_RULE_PATH = "bundled-rules/windows-git-bash.md";
const WINDOWS_GUIDANCE = "On Windows native Asterline sessions, prefer Git Bash for shell commands.";
...
function makeProject(): { readonly root: string; readonly pluginData: string } {
	...
	const root = mkdtempSync(join(tmpdir(), "asterline-rules-windows-git-bash-project-"));
	const pluginData = mkdtempSync(join(tmpdir(), "asterline-rules-windows-git-bash-data-"));
...
describe("Windows Git Bash bundled rule content", () => {
	...
	it("#given the bundled rule #when read #then it prefers the git_bash MCP and the absolute Git Bash path", () => {
		...
		expect(body).toContain("ASTERLINE_GIT_BASH_PATH");
...
describe("Windows Git Bash bundled rule", () => {
	...
	it("#given project rule with same description on Windows #when static rules load #then project guidance file overrides bundled guidance", async () => {
		...
		const projectRulePath = join(root, ".asterline", "rules", "windows-git-bash.md");
