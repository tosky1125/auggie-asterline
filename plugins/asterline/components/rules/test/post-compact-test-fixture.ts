...
import type { AsterlinePostCompactInput, AsterlineSessionStartInput, AsterlineUserPromptSubmitInput } from "../src/asterline-hook.js";
...
export const EXPANDED_POST_COMPACT_ENV = {
	...PROJECT_RULES_ENV,
	ASTERLINE_RULES_POST_COMPACT_MAX_RESULT_CHARS: "20000",
	ASTERLINE_RULES_POST_COMPACT_MAX_RULE_CHARS: "12000",
};
...
export function makeOversizedProject(prefix = "budget"): { root: string; pluginData: string } {
	const root = mkdtempSync(path.join(tmpdir(), `asterline-rules-post-compact-${prefix}-project-`));
	const pluginData = mkdtempSync(path.join(tmpdir(), `asterline-rules-post-compact-${prefix}-data-`));
...
export function sessionStartInput(root: string, sessionId = DEFAULT_SESSION_ID): AsterlineSessionStartInput {
	return {
...
export function postCompactInput(root: string, sessionId = DEFAULT_SESSION_ID): AsterlinePostCompactInput {
	return {
		session_id: sessionId,
...
export function userPromptSubmitInput(
	...
	transcriptPath: string,
	sessionId = DEFAULT_SESSION_ID,
): AsterlineUserPromptSubmitInput {
	return {
		session_id: sessionId,
