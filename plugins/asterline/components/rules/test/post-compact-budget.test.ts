...
describe("post-compact context budget", () => {
	...
	it("#given Asterline canonical context-window marker after compaction #when resolving post-compact budget #then shrinks projected rule injection", () => {
		// given
		const transcriptPath = writeCompactedAsterlineContextWindowTranscript("small compacted summary");
...
function writeCompactedAsterlineContextWindowTranscript(retainedText: string): string {
	const root = mkdtempSync(path.join(tmpdir(), "post-compact-budget-"));
	tempDirectories.push(root);
