...
const projectRoot = "/tmp/asterline-rules-bundled-priority";
...
describe("plugin bundled rule priority", () => {
	...
	it("#given aggregate plugin root #when resolving rules root #then components rules directory is selected", () => {
		// given
		const aggregateRoot = mkdtempSync(join(tmpdir(), "asterline-rules-aggregate-plugin-"));
		const componentRoot = join(aggregateRoot, "components", "rules");
		tempDirectories.push(aggregateRoot);
		mkdirSync(join(aggregateRoot, ".augment-plugin"), { recursive: true });
		mkdirSync(componentRoot, { recursive: true });
		writeFileSync(join(aggregateRoot, ".augment-plugin", "plugin.json"), JSON.stringify({ name: "asterline" }));
