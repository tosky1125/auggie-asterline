...
function makeAgentDocProject(): AgentDocProject {
	const root = mkdtempSync(path.join(tmpdir(), "asterline-rules-agent-docs-"));
	const pluginData = mkdtempSync(path.join(tmpdir(), "asterline-rules-agent-docs-data-"));
	tempDirectories.push(root, pluginData);

	writeFileSync(path.join(root, "package.json"), JSON.stringify({ name: "fixture" }));
	writeFileSync(path.join(root, "AGENTS.md"), "Root AGENTS.md must remain Asterline-native.");
	...
	writeFileSync(
		...
		[
			...
			"---",
			"",
			"Dynamic .asterline/rules context must not leak through removed-only allowlists.",
		].join("\n"),
	);

	const nestedDirectory = path.join(root, "child", "src");
	mkdirSync(nestedDirectory, { recursive: true });
	writeFileSync(path.join(root, "child", "AGENTS.md"), "Child AGENTS.md must remain Asterline-native.");
...
describe("agent doc sources", () => {
	for (const sourceList of REMOVED_AGENT_DOC_SOURCE_LISTS) {
		it(`#given ${sourceList} removed-only source allowlist #when SessionStart runs #then it emits no ASTERLINE rules context`, async () => {
			// given
			...
			const output = await runSessionStartHook(sessionStartInput(root), {
				...
				env: { ASTERLINE_RULES_ENABLED_SOURCES: sourceList },
			});
	...
	it("#given nested AGENTS.md and removed-only source allowlist #when PostToolUse targets a child file #then it emits no dynamic ASTERLINE rules context", async () => {
		// given
		...
		const output = await runPostToolUseHook(postToolUseInput(root, nestedSourceFile), {
			pluginDataRoot: pluginData,
			env: { ASTERLINE_RULES_ENABLED_SOURCES: "AGENTS.md,CLAUDE.md" },
		});
