...
const projectRoot = "/tmp/asterline-rules-engine";

function makeCandidate(overrides: Partial<RuleCandidate> = {}): RuleCandidate {
	const candidate = {
		path: join(projectRoot, ".asterline", "rules", "typescript.md"),
		realPath: join(projectRoot, ".asterline", "rules", "typescript.md"),
		source: ".asterline/rules",
		distance: 0,
		isGlobal: false,
		isSingleFile: false,
		relativePath: ".asterline/rules/typescript.md",
...
describe("rule engine default source selection", () => {
	it("#given auto source selection #when loading static rules #then Asterline-native and Claude-home sources are disabled by default", () => {
		// given
	...
	it("#given removed agent-doc sources and a real source are requested #when loading static rules #then only real sources are enabled", () => {
		...
		const engine = createEngine(
			configFromEnvironment({ ASTERLINE_RULES_ENABLED_SOURCES: "AGENTS.md,~/.claude/CLAUDE.md,plugin-bundled" }),
			deps,
		);
		...
		expect(capturedDisabledSources?.has(".asterline/rules")).toBe(true);
	});
});
...
describe("rule engine static loading", () => {
	...
	it("#given project candidate resolves outside project #when loading static rules #then the rule is skipped with a diagnostic", () => {
		// given
		const outsidePath = "/tmp/asterline-rules-outside/.asterline/rules/typescript.md";
