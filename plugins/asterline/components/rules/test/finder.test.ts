...
function makeProject(): { projectRoot: string; homeRoot: string; targetPath: string } {
	const projectRoot = mkdtempSync(join(tmpdir(), "asterline-rules-finder-project-"));
	const homeRoot = mkdtempSync(join(tmpdir(), "asterline-rules-finder-home-"));
	tempDirectories.push(projectRoot, homeRoot);
	mkdirSync(join(projectRoot, "src", ".asterline", "rules"), { recursive: true });
	...
	writeFileSync(join(projectRoot, "src", ".asterline", "rules", "local.md"), "Local rule\n");
	writeFileSync(join(projectRoot, ".asterline", "rules", "root.md"), "Root rule\n");
...
describe("findRuleCandidates", () => {
	it("#given project and user-home rules #when target file is inside project #then candidates keep source distance", () => {
		...
		expect(candidates.map(candidateSummary)).toEqual([
			".asterline/rules:0:src/.asterline/rules/local.md",
			".asterline/rules:1:.asterline/rules/root.md",
	...
	it("#given disabled source #when finding candidates #then matching source is omitted", () => {
		...
		const candidates = findRuleCandidates({
			...
			targetFile: targetPath,
			homeDir: homeRoot,
			disabledSources: new Set([".asterline/rules", "~/.opencode/rules", "plugin-bundled"]),
		});
