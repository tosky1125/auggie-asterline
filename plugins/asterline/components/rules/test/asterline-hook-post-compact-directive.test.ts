...
describe("asterline rules post-compaction read directive", () => {
	it("#given injected rules dropped by compaction #when UserPromptSubmit recovers #then it lists rule paths in a mandatory read directive instead of bodies", async () => {
		...
		expect(context).toContain(".asterline/rules/typescript.md");
	...
	it("#given bundled hephaestus rule dropped by compaction #when compact SessionStart recovers #then hephaestus body is re-injected in full alongside the directive", async () => {
		// given
		const { root, pluginData } = makeProject();
		const env = { ASTERLINE_RULES_ENABLED_SOURCES: "CONTEXT.md,plugin-bundled" };
...
function makeProject(): { root: string; pluginData: string } {
	const root = mkdtempSync(path.join(tmpdir(), "asterline-rules-directive-project-"));
	const pluginData = mkdtempSync(path.join(tmpdir(), "asterline-rules-directive-data-"));
...
function compactSessionStartInput(root: string, transcriptPath: string): AsterlineSessionStartInput {
	return {
		session_id: SESSION_ID,
...
function userPromptSubmitInput(root: string, transcriptPath: string): AsterlineUserPromptSubmitInput {
	return {
		session_id: SESSION_ID,
