...
describe("rules source selection", () => {
	...
	it("#given explicit enabled sources #when disabled source set is derived #then all other known sources are omitted", () => {
		...
		const config: PiRulesConfig = {
			...defaultConfig(),
			enabledSources: [".asterline/rules", "plugin-bundled"],
		};
		...
		// then
		for (const source of [".asterline/rules", "plugin-bundled"]) {
		...
		expect(disabledSources).toEqual(
			new Set(
				[...SOURCE_PRIORITY.keys()].filter((source) => source !== ".asterline/rules" && source !== "plugin-bundled"),
			),
		);
	});
});
