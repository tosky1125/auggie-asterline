...
describe("rules formatter hook context", () => {
	it("#given multiline dynamic rules #when formatting PostToolUse context #then labels and bodies render on separate lines", () => {
		...
		const block = formatDynamicBlock(
			[rule],
			"packages/asterline-runtime/plugin/components/work-loop/src/paths.ts",
			FORMAT_OPTIONS,
		);
		...
		expect(block).toBe(
			[
				"Additional project instructions matched for packages/asterline-runtime/plugin/components/work-loop/src/paths.ts:",
				"",
