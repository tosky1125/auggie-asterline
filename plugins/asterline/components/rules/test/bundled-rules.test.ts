...
const BUNDLED_ONLY_ENV = {
	ASTERLINE_RULES_ENABLED_SOURCES: "plugin-bundled",
	ASTERLINE_RULES_MAX_RESULT_CHARS: "40000",
};

const PROJECT_AND_BUNDLED_ENV = {
	ASTERLINE_RULES_ENABLED_SOURCES: ".asterline/rules,plugin-bundled",
	ASTERLINE_RULES_MAX_RESULT_CHARS: "40000",
};
...
const DISABLED_BUNDLED_ENV = {
	ASTERLINE_RULES_ENABLED_SOURCES: "plugin-bundled",
	ASTERLINE_RULES_MAX_RESULT_CHARS: "40000",
	ASTERLINE_RULES_DISABLE_BUNDLED: "1",
};
...
function makeFixture(options: FixtureOptions = {}): Fixture {
	const root = mkdtempSync(join(tmpdir(), "asterline-rules-bundled-project-"));
	const pluginRoot = mkdtempSync(join(tmpdir(), "asterline-rules-bundled-plugin-"));
	const pluginData = mkdtempSync(join(tmpdir(), "asterline-rules-bundled-data-"));
	tempDirectories.push(root, pluginRoot, pluginData);

	writeFileSync(join(root, "package.json"), JSON.stringify({ name: "fixture" }));
	mkdirSync(join(root, ".asterline", "rules"), { recursive: true });
	...
	const projectRulePath = join(root, ".asterline", "rules", "hephaestus.md");
...
describe("plugin bundled rules", () => {
	...
	it("#given bundled Hephaestus rule body exceeds per-rule cap #when SessionStart runs #then static context expands the body within result budget", async () => {
		// given
		const root = mkdtempSync(join(tmpdir(), "asterline-rules-bundled-large-project-"));
		const pluginRoot = mkdtempSync(join(tmpdir(), "asterline-rules-bundled-large-plugin-"));
		const pluginData = mkdtempSync(join(tmpdir(), "asterline-rules-bundled-large-data-"));
		tempDirectories.push(root, pluginRoot, pluginData);
		writeFileSync(join(root, "package.json"), JSON.stringify({ name: "fixture" }));
		mkdirSync(join(pluginRoot, "bundled-rules"), { recursive: true });
	...
	it("#given project rule body exceeds per-rule cap #when SessionStart runs #then static context injects a truncated body", async () => {
		// given
		const root = mkdtempSync(join(tmpdir(), "asterline-rules-project-large-project-"));
		const pluginRoot = mkdtempSync(join(tmpdir(), "asterline-rules-project-large-plugin-"));
		const pluginData = mkdtempSync(join(tmpdir(), "asterline-rules-project-large-data-"));
		tempDirectories.push(root, pluginRoot, pluginData);
		writeFileSync(join(root, "package.json"), JSON.stringify({ name: "fixture" }));
		mkdirSync(join(root, ".asterline", "rules"), { recursive: true });
		mkdirSync(join(pluginRoot, "bundled-rules"), { recursive: true });
		...
		const projectRulePath = join(root, ".asterline", "rules", "oversized.md");
