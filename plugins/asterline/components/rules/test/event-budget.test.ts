...
describe("asterline rules per-event injection budgets", () => {
	...
	it("#given dynamic budget env overrides #when PostToolUse injects #then the override budget wins", async () => {
		...
		const output = await runPostToolUseHook(postToolUseInput(root, path.join(root, "src", "app.ts")), {
			...
			env: {
				...PROJECT_ONLY_ENV,
				ASTERLINE_RULES_DYNAMIC_MAX_RULE_CHARS: "1000",
				ASTERLINE_RULES_DYNAMIC_MAX_RESULT_CHARS: "1500",
			},
		});
...
function makeProject(sizes: { staticRuleChars?: number; dynamicRuleChars?: number }): {
	...
	const root = mkdtempSync(path.join(tmpdir(), "asterline-rules-event-budget-project-"));
	const pluginData = mkdtempSync(path.join(tmpdir(), "asterline-rules-event-budget-data-"));
...
function userPromptSubmitInput(root: string): AsterlineUserPromptSubmitInput {
	return {
		session_id: `session-budget-${path.basename(root)}`,
		turn_id: "turn-1",
		transcript_path: null,
