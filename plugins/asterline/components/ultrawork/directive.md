...
Start a host goal (or open your reply with a `# Goal` block treated as
...
the WHOLE notepad FIRST — `asterline sparkshell cat "$NOTE"`, or read the path
...
The todo tool is Asterline `update_plan` — your live, user-visible
...
- Repo-wide inspection, CLI smoke tests, git/history, bounded command
  output → prefer `asterline sparkshell <command>` before raw shell commands
  (use `asterline sparkshell --shell '<cmd>'` only when shell metacharacters
