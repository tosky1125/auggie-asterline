...
Asterline plugin that injects a compact orchestration directive (the **ultrawork** prompt) when the user prompt contains `ultrawork` or `ulw` (word-bounded, case-insensitive).

Bundled Asterline agent role TOMLs in `agents/` are installed into `ASTERLINE_HOME/agents/` by the asterline-runtime installer (`linkCachedPluginAgents`, in `src/cli/install-asterline/link-cached-plugin-agents.ts`). Install-time writes regular file copies on every platform. For the public `sisyphuslabs` marketplace, those files are copied from Asterline's local installed-marketplace snapshot so they keep resolving after Asterline prunes old plugin-cache versions or temporary marketplace state. There is no runtime Python hook.

## What the injected directive enforces

| Mandate | Behavior |
|---|---|
| Goal + binding success criteria | Start a host goal (or open with a `# Goal` block) listing the deliverable + **3+ realistic QA scenarios** (happy path, edge cases, adjacent-surface regression). Each scenario MUST name which **Manual-QA channel** it will use. "Tests pass" is supporting signal, NEVER completion proof. |
...
| ChatGPT-compatible high-reasoning verification gate | Triggered automatically on user-requested rigor, 3+ files, 20+ turns, 30+ minutes, or refactor/migration/perf/security work. Use the bundled `asterline-work-reviewer` agent role when available. Reviewer verdict is **binding**: no "false positive", no minimising, no arguing. Loop until **unconditional** approval. "Looks good but..." = REJECTION. |
...
The installer copies the plugin into `~/.asterline/plugins/cache/sisyphuslabs/asterline/0.1.0`, writes the Asterline marketplace snapshot at `~/.asterline/.tmp/marketplaces/sisyphuslabs/`, registers the `sisyphuslabs` marketplace from the `asterline` Git repository, enables `asterline@sisyphuslabs` in `~/.asterline/config.toml`, registers the `UserPromptSubmit` hook, and installs the bundled agent TOMLs as regular files under `~/.asterline/agents/`. A `.installed-agents.json` manifest is written next to the bundled TOMLs' source root for clean uninstall tracking.
...
Asterline passes the prompt payload on stdin. When the pattern `\b(?:ultrawork|ulw)\b` (case-insensitive) matches, the hook writes the directive to stdout — Asterline injects non-JSON stdout as `additional_context` for the next turn. Otherwise the hook writes nothing and exits 0. Malformed input also exits 0 to never block the turn.
...
Bundled agent role TOMLs in `agents/` ship to `ASTERLINE_HOME/agents/` at install time, not via a runtime hook. The installer writes regular file copies on Linux, macOS, and Windows. For the public marketplace, the source is the installed-marketplace snapshot, not the versioned plugin cache, so agent role configs remain valid when Asterline replaces `~/.asterline/plugins/cache/sisyphuslabs/asterline/<version>/` during auto-update or removes temporary marketplace state. Both code paths overwrite stale files and write a `.installed-agents.json` manifest next to the source root for clean uninstall tracking.
...
Run `npx asterline-ai install`, then inspect `~/.asterline/agents/`. On every platform you should see regular `.toml` files. Each TOML should declare a non-empty `name`, `description`, and `developer_instructions`.
...
This plugin only reads local hook payloads and emits the bundled directive text on keyword match. Bundled agent TOML files ship to `ASTERLINE_HOME/agents/` at install time. No network calls and no telemetry from this component.
