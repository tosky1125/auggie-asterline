...
import { isUltraworkPrompt, runUserPromptSubmitHook } from "../src/asterline-hook.js";
...
describe("asterline ultrawork hook", () => {
	it("#given ultrawork prompt #when hook runs #then emits directive as Asterline hook JSON", () => {
		// given
		const payload = {
	...
	it("#given Windows cwd #when hook sees ultrawork prompt #then emits directive as Asterline hook JSON", () => {
		// given
		const payload = {
			cwd: "C:\\Users\\asterline\\project",
			hook_event_name: "UserPromptSubmit",
			model: "gpt-5.5",
	...
	it("#given Asterline canonical context-window transcript with ulw prompt #when hook runs #then does not add more context", () => {
		// given
		const payload = {
			hook_event_name: "UserPromptSubmit",
			prompt: "please ulw this change",
			transcript_path: writeAsterlineContextWindowTranscript(),
		};
	...
	it("#given directive #when inspected #then hardens Asterline subagent assignment ambiguity", () => {
		// given
		const payload = {
			hook_event_name: "UserPromptSubmit",
			prompt: "please ultrawork",
...
function writeContextPressureTranscript(): string {
	const root = mkdtempSync(path.join(tmpdir(), "asterline-ultrawork-context-pressure-"));
...
function writeTranscript(...lines: string[]): string {
	const root = mkdtempSync(path.join(tmpdir(), "asterline-ultrawork-transcript-"));
	tempDirectories.push(root);
	const transcriptPath = path.join(root, "transcript.jsonl");
...
function writeAsterlineContextWindowTranscript(): string {
	const root = mkdtempSync(path.join(tmpdir(), "asterline-ultrawork-context-window-"));
	tempDirectories.push(root);
	const transcriptPath = path.join(root, "transcript.jsonl");
	writeFileSync(
		transcriptPath,
