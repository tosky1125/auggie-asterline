...
describe("start-work continuation CLI", () => {
	...
	it("#given unrelated session stdin #when CLI runs #then stdout is empty and exit is zero", () => {
		// given
		const cwd = createWorkspace(["asterline:other"]);
		const payload = JSON.stringify(makePayload(cwd, false, "Stop"));
...
function createWorkspace(sessionIds: readonly string[]): string {
	const root = mkdtempSync(join(tmpdir(), "asterline-continuation-cli-"));
	cleanupRoots.push(root);
	mkdirSync(join(root, ".asterline", "plans"), { recursive: true });
	writeFileSync(join(root, ".asterline", "plans", "plan.md"), "## TODOs\n\n- [ ] Task one\n");
	const work = {
		work_id: "w1",
	...
	writeFileSync(
		join(root, ".asterline", "boulder.json"),
