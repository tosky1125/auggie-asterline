...
{{WORKTREE_BLOCK}}
- Ledger: `{{LEDGER_PATH}}`
- Your session id in boulder.json: `asterline:{{SESSION_ID}}`
...
3. Follow the `start-work` skill in full. The skill is already loaded from your earlier turn — re-read its file at `packages/asterline-runtime/plugin/skills/start-work/SKILL.md` if you have lost context.
...
- session_ids you write to boulder.json MUST be prefixed `asterline:`. Bare ids on read are legacy `opencode:`.
