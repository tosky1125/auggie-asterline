...
async function makeFixture() {
	const root = await mkdtemp(join(tmpdir(), "asterline-lsp-build-"));
	await mkdir(join(root, "packages", "asterline-runtime", "plugin", "components", "lsp", "scripts"), { recursive: true });
	await mkdir(join(root, "packages", "lsp-tools-mcp", "dist"), { recursive: true });
	await copyFile(
		new URL("./build-lsp-tools.mjs", import.meta.url),
		join(root, "packages", "asterline-runtime", "plugin", "components", "lsp", "scripts", "build-lsp-tools.mjs"),
	);
	...
	return { root, npmLog, script: join(root, "packages", "asterline-runtime", "plugin", "components", "lsp", "scripts", "build-lsp-tools.mjs"), fakeBin };
}
