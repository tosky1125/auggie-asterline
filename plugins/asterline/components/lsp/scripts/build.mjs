...
if (configuredBun === undefined && (probe?.error !== undefined || probe?.status !== 0 || probe.stdout.trim().length === 0)) {
  throw new Error(`Bun 1.3.14 is required to reproduce the LSP hook bundle: ${probe?.error?.message ?? probe?.stderr.trim()}`);
}
...
await bundleComponent({
  ...
  config: {
    ...
    entries: [
      { source: 'components/lsp/src/cli.ts', output: 'cli.js', executable: true },
      { source: 'components/lsp/src/asterline-hook.ts', output: 'asterline-hook.js' },
    ],
    aliases: [],
  },
});
