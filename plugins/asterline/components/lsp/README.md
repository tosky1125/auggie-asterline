...
[![ci](https://github.com/code-yeongyu/asterline-lsp/actions/workflows/ci.yml/badge.svg)](https://github.com/code-yeongyu/asterline-lsp/actions/workflows/ci.yml) [![license: MIT](https://img.shields.io/badge/license-MIT-blue.svg)](LICENSE)

Asterline plugin that ports the standalone LSP runtime from [`pi-lsp-client`](https://github.com/code-yeongyu/pi-lsp-client). It gives Asterline post-edit diagnostics plus explicit MCP tools for language-aware code work.
...
- `asterline-lsp` keeps Asterline-specific integration (`hook post-tool-use`, plugin metadata, package wiring).
...
| diagnostics contain errors | returns Asterline `PostToolUse` blocking feedback and injects the same diagnostics as additional context so Asterline fixes the file |
...
`@code-yeongyu/lsp-tools-mcp/dist/*.js` is available for the asterline-lsp build.
...
The installer builds and copies the plugin into `~/.asterline/plugins/cache/sisyphuslabs/asterline/0.1.0`, registers the `sisyphuslabs` marketplace from the `asterline` Git repository, and enables:
...
- [pi-lsp-client](https://github.com/code-yeongyu/pi-lsp-client) - source extension this Asterline plugin ports.
