...
import { runLspPostCompactHook, runLspPostToolUseHook } from "../src/asterline-hook.js";
...
describe("asterline PostToolUse unavailable LSP suppression", () => {
	it("#given unavailable markdown LSP in one session #when PostToolUse repeats #then suppresses feedback and skips the cached extension", async () => {
		...
		const input = postToolUseInput("session-unavailable", ".asterline/work-loop/evidence/note.md");
		let calls = 0;
	...
	it("#given cached unavailable LSP after PostCompact #when the next PostToolUse runs #then probes once and suppresses again", async () => {
		// given
		const pluginData = tempPluginData();
		const input = postToolUseInput("session-compact", ".asterline/work-loop/evidence/note.md");
		let calls = 0;
	...
	it("#given cached unavailable LSP after PostCompact #when the probe is clean #then clears the unavailable cache", async () => {
		// given
		const pluginData = tempPluginData();
		const input = postToolUseInput("session-compact-clean", ".asterline/work-loop/evidence/note.md");
		let calls = 0;
	...
	it("#given a cached unavailable extension #when the post-compact probe hits a daemon outage #then the cache survives and the probe stays pending", async () => {
		// given
		const pluginData = tempPluginData();
		const input = postToolUseInput("session-daemon-down-probe", ".asterline/work-loop/evidence/note.md");
		let calls = 0;
...
function tempPluginData(): string {
	const dir = mkdtempSync(path.join(tmpdir(), "asterline-lsp-unavailable-"));
	tempDirs.push(dir);
	return dir;
}
