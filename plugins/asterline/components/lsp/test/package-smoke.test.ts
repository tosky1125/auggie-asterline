...
describe("plugin package metadata", () => {
	it("#given packaged component files #when validating entrypoints #then hook command stays local and MCP command references the package", () => {
		...
		const asterlineHookCliSource = readFileSync("src/asterline-hook-cli.ts", "utf8");
		const asterlineHookSource = readFileSync("src/asterline-hook.ts", "utf8");
		...
		expect(packageJson.bin["asterline-lsp"]).toBe("./dist/cli.js");
		expect(packageJson.bin["asterline-lsp"]).toBeUndefined();
		expect(packageJson.scripts["build"]).toBe("node scripts/clean-dist.mjs && tsc -p tsconfig.build.json");
		expect(cliSource.startsWith("#!/usr/bin/env node")).toBe(true);
		expect(cliSource).toContain("Usage: asterline-lsp [mcp | hook post-tool-use | hook post-compact]");
		...
		expect(asterlineHookCliSource).toContain("@code-yeongyu/lsp-daemon");
		expect(asterlineHookSource).toContain("@code-yeongyu/lsp-daemon");
		expect(asterlineHookCliSource).not.toContain("../../../../../lsp-daemon");
		expect(asterlineHookSource).not.toContain("../../../../../lsp-daemon");
