...
import { extractMutatedFilePaths, runLspPostToolUseHook } from "../src/asterline-hook.js";
...
describe("asterline PostToolUse hook", () => {
	it("extracts files from Asterline apply_patch command payloads", () => {
	...
	it("#given Asterline canonical context-window transcript and large diagnostics #when the hook blocks #then it caps injected feedback", async () => {
		const root = mkdtempSync(path.join(tmpdir(), "asterline-lsp-context-pressure-"));
		tempDirs.push(root);
