...
function getInstallDecisionsPath() {
  ...
  if (!override)
    return join2(homedir(), ".asterline", "lsp-install-decisions.json");
...
function getProjectConfigPaths() {
  ...
  return [join3(contextCwd(), ".asterline", "lsp-client.json")];
}
function getUserConfigPath() {
  const userOverride = contextEnv("LSP_TOOLS_MCP_USER_CONFIG");
  if (!userOverride)
    return join3(homedir2(), ".asterline", "lsp-client.json");
...
function formatServerLookupError(result) {
  ...
  return [
    ...
    "",
    "Configure a custom server in '.asterline/lsp-client.json':",
    "  {",
    '    "lsp": {',
    '      "my-server": {',
...
function daemonBaseDir(env = process.env) {
  ...
  const home = asterlineHome && asterlineHome.length > 0 ? asterlineHome : join8(homedir3(), ".asterline");
  return join8(home, "asterline-lsp", "daemon");
}
...
function resolveSocketPath(dir, version) {
  ...
  if (process.platform === "win32") {
    return `\\\\.\\pipe\\asterline-code-intel-${version}-${digest}`;
  }
  const natural = join8(dir, "daemon.sock");
  if (natural.length < MAX_SOCKET_PATH_LENGTH)
    return natural;
  return join8(tmpdir(), `asterline-code-intel-${version}-${digest}.sock`);
}
...
async function main() {
  ...
  stderr.write(`Usage: asterline-code-intel-daemon [mcp | daemon]
`);
  process.exitCode = 2;
}
main().catch((error) => {
