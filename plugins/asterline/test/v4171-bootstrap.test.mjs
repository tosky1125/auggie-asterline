...
test("Given the v4.19.3 port, when metadata is inspected, then provenance and Auggie hook fields are exact", () => {
...
test("Given the shipped bundle, when invoked, then it is self-contained and exposes the current runtime", () => {
	...
	assert.match(result.stdout, /Asterline bootstrap 4\.19\.3/);
...
test("Given a stale bootstrap lock, when SessionStart runs, then it replaces the lock through a detached worker", async () => {
	...
	await writeFile(join(root, ".augment-plugin", "plugin.json"), '{"version":"4.19.3"}');
