...
if command -v asterline >/dev/null 2>&1 && asterline work-loop help >/dev/null 2>&1; then
  ...
  for candidate in "$HOME/.local/bin/asterline" "$ASTERLINE_HOME/bin/asterline" "$ASTERLINE_HOME"/plugins/cache/*/asterline/*/components/*/dist/cli.js; do
...
if [ -z "${WORK_LOOP_CLI:-}" ]; then
  ...
  printf '%s\n' "No work-loop-capable asterline executable found; PATH asterline may be the OpenCode CLI without the Auggie work-loop subcommand, and cached work-loop CLI was not found under ${ASTERLINE_HOME:-$HOME/.asterline}." >> "$NOTE" 2>/dev/null || true
  printf '%s\n' "Install the Asterline marketplace plugin or set ASTERLINE_LOCAL_BIN_DIR to a PATH directory containing the asterline executable." >&2
fi
```
