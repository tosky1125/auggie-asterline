# Optional React Dev Tooling

When working on a React project, first detect whether these dev-only tools are already provisioned. Asterline never installs them automatically or invokes a package manager. The commands below are operator-run options to present only when the user chooses to add project dependencies.
...
After an operator completes setup, confirm by reading the diff. Each tool should appear ONLY behind a `process.env.NODE_ENV === "development"` / `import.meta.env.DEV` gate.
...
1. **As an agent skill** so your coding agent learns from each scan and avoids the issues next time:
   ...
   The upstream CLI may detect its supported agent products. Do not assume it can install an Auggie plugin; use its project-local scan only unless the operator verifies otherwise.
