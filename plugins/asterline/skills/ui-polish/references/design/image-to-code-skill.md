---
name: image-to-code
description: Elite website image-to-code skill for Auggie. For visually important web tasks, it must first generate the design image(s) itself, deeply analyze them, then implement the website to match them as closely as possible. In Auggie, it must prefer large, readable, section-specific images instead of tiny compressed boards, generate fresh standalone images for sections or detail views instead of cropping old ones, avoid lazy under-generation, avoid cards-inside-cards-inside-cards UI, and keep the hero clean, spacious, readable, and visible on a small laptop.
---
...
- In Auggie, bias strongly toward larger, more analyzable section images.
...
## 4. AUGGIE-SPECIFIC SECTION IMAGE RULE

Inside Auggie, do not compress too many website sections into one single image if that would make the text, spacing, buttons, or layout details too small to analyze properly.

In Auggie, prefer separate large images per section.

Default rule inside Auggie:
...
Outside Auggie, this skill may still allow more compact multi-section composition when appropriate.
Inside Auggie, prioritize section clarity and extraction accuracy.
...
## 10. IMAGE-FIRST AUGGIE WEBSITE WORKFLOW

When this skill is used inside Auggie or any environment that supports image generation plus implementation, default to an image-first workflow for website design tasks.
...
Inside Auggie, treat each section as its own analyzable unit.
...
In Auggie, these should usually become section-by-section images, not one compressed sheet.
...
17. Has Auggie avoided compressing too many sections into one tiny image?
...
4. inside Auggie, prefer one large image per section
...
Do not compress many sections into one unreadable image in Auggie.
...
Interpretation:
- generate 8 separate section images in Auggie
- one per section
...
Interpretation:
- generate 4 separate section images in Auggie
...
Inside Auggie, if the user wants multiple sections, prefer separate large section images instead of one compressed multi-section board, so text, spacing, typography, buttons, and colors can be extracted properly.
