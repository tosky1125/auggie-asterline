...
The generated corpus already includes the licensed Open Design references selected for this release. Search `_INDEX.md`; do not claim that a separate design plugin or skill is installed in Auggie.
...
3. **Greenfield setup:** if the user gave no concrete visual reference, use `_INDEX.md` to shortlist 2-3 plausible Layer B references, then read exactly one Layer A style skill and one Layer B brand/design-system reference in full. Open `DESIGN.md` with a `## 0. Research Log` recording each completed lane and every unavailable browser or image-generation lane. Treat those references as source material, not mood labels: extract tokens, layout grammar, component anatomy, interaction states, motion, and taste decisions into `DESIGN.md`, then recombine them into project-specific primitives. Customize for the user's product and content, but do not freestyle past the selected references; never copy logos, trademarked assets, or brand-specific copy.
...
## Phase 0.5 — Optional React Diagnostics Check

If the project ships React (`react` in `package.json`), check whether react-grab, react-scan, and react-doctor are already provisioned. Asterline never installs them automatically and never invokes a package manager on the user's behalf.
...
#### If NO → REPORT OPTIONAL OPERATOR COMMANDS

The following upstream commands may be shown to the user as optional setup instructions. Run them only after the user explicitly chooses to provision these project dependencies:
...
When these diagnostics are unavailable, record the skipped diagnostic lane and continue with the checks the current Auggie environment can actually run. Never report those tools as passing when they were not provisioned.
...
**Action when image generation is available in the current Auggie environment:** Load both:
...
For an expressive greenfield brief, generate **2-3 concept drafts** only when image generation is configured. Otherwise record that the lane is unavailable and continue from bundled references or user-provided images; never invent draft evidence.
...
Before declaring the task done, verify the rendered UI through `/visual-check` when browser tooling is configured. It should capture every page and breakpoint (375 / 768 / 1280px), interaction states, and motion. If no browser capability is available, use user-provided captures and static checks, explicitly record the missing rendered-evidence lane, and do not claim `/visual-check` browser coverage passed.

This skill adds only the design-taste judgments `/visual-check` cannot make for you:
