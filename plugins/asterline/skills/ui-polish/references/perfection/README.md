...
Use configured Auggie browser tooling when available. If no browser capability is provisioned, record the rendered-audit lane as unavailable and do not claim a Lighthouse result.
...
If the project ships React and the diagnostics are already provisioned, Lighthouse alone does NOT see render-layer issues by component. Also run:

- **`react-doctor`** (static): use the existing project-local command before a browser audit. The upstream `npx react-doctor@latest --json` form is an optional operator command, never an automatic Asterline action.
...
Optional operator setup snippets live in `../design/react-dev-tooling-skill.md`. If the tools are absent, report that lane as unavailable rather than installing them.
