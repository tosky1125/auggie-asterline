...
This is an internal frontend ruleset, not a standalone skill. `ui-polish` remains the only public activation point for web UI, UX, visual design, accessibility, design QA, and frontend implementation routing.
...
- `/visual-check` still owns objective rendered evidence for visual claims.
- `/work-plan`, `/run-plan`, and `/review-pass` still own planning, execution, and final implementation review when those phases apply.
...
Do not load this reference instead of the frontend skill. It only supplies design routing and state language inside `ui-polish`.
...
- significant implementation work routes through `/review-pass`.
