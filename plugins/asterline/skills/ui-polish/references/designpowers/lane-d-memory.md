...
Lane D records the design record around Asterline work. It owns `.asterline/ui-polish-design/state.md`, design debt, handoff notes, retrospective notes, and observational taste memory. It has no hooks and no independent automation path. It records and summarizes what happened so the next Asterline phase has reliable context.

## Phase Owner

| Capability | Source boundary | owner | Mapping |
|---|---|---|---|
| Track deferred design and accessibility findings | `design-debt-tracker` | `.asterline/ui-polish-design/state.md` plus final `review-pass` context | Maintain a register with ID, date, source, severity, issue, affected users, suggested fix, status, and notes. |
| Package design rationale for implementers or reviewers | `design-handoff` | `run-plan` and `review-pass` context | Record component, interaction, accessibility, content, and rationale notes that workers or reviewers need. |
...
| Maintain observational design memory | Authored concept preserved from excluded raw `design-memory` | `.asterline/ui-polish-design/state.md` and optional personal report context | Store observations descriptively. Do not feed them back as future project constraints. |
...
```text
.asterline/ui-polish-design/state.md
```
...
Update `.asterline/ui-polish-design/state.md` with:
...
- `.asterline/ui-polish-design/state.md` exists before a lane claims durable state.
...
- The evidence index points to real files produced by Asterline planning, `run-plan`, `visual-check`, or `review-pass`.
...
PASS when state, design debt, handoff, retrospective, and evidence references are current enough for `work-plan`, `run-plan`, `visual-check`, or `review-pass` to resume without guessing.
