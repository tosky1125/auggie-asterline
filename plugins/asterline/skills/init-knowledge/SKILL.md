...
```
/init-knowledge                      # Update mode: modify existing + create new where warranted
/init-knowledge --create-new         # Read existing → remove all → regenerate from scratch
/init-knowledge --max-depth=2        # Limit directory depth (default: 3)
```
