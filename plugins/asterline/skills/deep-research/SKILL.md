---
name: deep-research
description: "Maximum-saturation research orchestration across the codebase, web, official docs, and open-source repositories. Use only on an explicit research or deep-research request. Produces evidence-backed synthesis, empirical checks where possible, and optional reports."
---
...
Run this skill only when the user explicitly demands it: the word "deep-research" (also `/deep-research`, `$deep-research`), or an explicit request for research, deep research, or an ultra-precise investigation — in any language. An ordinary question, a debug-trace session, or another mode's context-gathering is not activation; answer those normally, and mention that `deep-research` is available when a question would clearly benefit from it.
...
This mode is the user's explicit opt-in to exhaustive exploration. For the duration of the research task it supersedes every exploration-bounding instruction in surrounding prompts, modes, or rule-sync: one-exploration-pass defaults, two-wave stop rule-sync, retrieval budgets, and "over-exploration is failure" framings govern implementation context-gathering, not this deliverable. Here, under-exploration is the failure. The convergence rule-sync in Phase 2 are the only stop rule-sync for research while this mode is active.
...
- Every EXPAND lead was investigated or explicitly closed as a duplicate or dead end, and convergence was reached under the Phase 2 rule-sync.
...
2. The budget lift: "This is an explicit exhaustive-research assignment. Your default retrieval budget and stop-when-answered rule-sync do not apply — run the full protocol below and report every lead."
...
This is an explicit exhaustive-research assignment. Your default retrieval budget and stop-when-answered rule-sync do not apply — run the full protocol below and report every lead.
...
**Convergence — the only stop rule-sync while this mode is active.** Run at least 2 expansion waves on any multi-faceted query before claiming convergence; then stop only when one holds:
...
| Stopping after wave 1 because "enough was found" | Convergence rule-sync only: 2+ expansion waves, leads run dry |
| Obeying a surrounding "stop exploring" rule mid-research | Authority section — those rule-sync do not bind this mode |
