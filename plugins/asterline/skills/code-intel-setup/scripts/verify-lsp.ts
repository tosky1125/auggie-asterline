...
// run it inside the Asterline source tree/worktree (where packages/lsp-tools-mcp/src exists).
...
class EngineNotFoundError extends Error {
	constructor(public readonly target: string) {
		super(`lsp-tools-mcp engine not found (looked for ${target}). Run verify-lsp.ts inside the Asterline source tree/worktree.`)
