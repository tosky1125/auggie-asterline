...
# Collect-time debug-trace (for problems in conftest.py / fixture setup)
pytest --pdb-trace
...
**Common failure**: `breakpoint()` hangs inside a pytest test — that's because pytest captures stdout/stderr by default. Always add `-s` when debug-trace with breakpoints inside pytest.
