...
- **Interactive debug-trace** → [tools/pwndbg.md](../tools/pwndbg.md)
...
**Interactive debug-trace on macOS — use `lldb`, not `gdb`.**
...
## [4] Dynamic debug-trace with pwndbg
...
**Open [tools/pwndbg.md](../tools/pwndbg.md) before launching gdb.** Pwndbg gives you the context view (registers / stack / disasm / code all visible at once) which is essential for binary debug-trace.
