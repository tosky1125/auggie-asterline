...
**The default `cargo run` builds with `dev` profile** which includes debug symbols. `cargo run --release` strips them. For debug-trace, stay in dev unless the bug only manifests under optimization.

---

## The Rust debug-trace hierarchy (use in this order)
...
## `RUST_LOG` for flow-level debug-trace
...
## tokio-console — async task debug-trace
