...
Escalation web access for tasks Auggie's normal web search or fetch cannot complete. Reach for this skill the moment a page is blocked (WAF / 403 / Cloudflare), needs JS rendering, hides behind a login, or lives on a platform a generic fetcher cannot read. Escalate only when the cheaper tier cannot do the job. Asterline never installs tools or packages at runtime; optional external tools mentioned below must already be provisioned by the operator.
...
CloakBrowser is a stealth Chromium with source-level fingerprint patches that passes Cloudflare Turnstile, FingerprintJS, BrowserScan, and 30+ detectors; agent-browser is the CDP automation CLI that drives it. Neither tool is vendored or installed by Asterline. Full operator prerequisites, version pins, launch flow, cookie login, and cross-platform notes are in [references/chrome-stealth.md](references/chrome-stealth.md).

```bash
# 1. Launch the operator-provisioned CloakBrowser with CDP on :9242.
...
| [references/chrome-stealth.md](references/chrome-stealth.md) | Tier-2 CloakBrowser + agent-browser prerequisites, CDP flow, version pins, cookie login |
...
# Asterline performs no dependency installation; optional executors must already exist.
```
