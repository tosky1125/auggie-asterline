...
Auggie browser-tool invocation requires caller tool access; this module
provides the subprocess path for local JS templates but only stubs the
connector path, which must be driven by the active Auggie session.
"""
...
def _run_node_template(template: str, args: dict[str, str | int], timeout: int = 90) -> tuple[int, str, str]:
...
def run_playwright_fallback(
    ...
    *,
    ...
    if choice.startswith("playwright_mcp"):
        att.error = (
            "An Auggie browser connector must be invoked by the active session; "
            "this local engine cannot call connector tools."
        )
    ...
    if not _chrome_channel_available():
        att.error = "node is unavailable for the local Playwright template"
        att.verdict = Verdict.UNKNOWN.value
        att.elapsed_s = round(time.time() - t0, 3)
        return att, ""
