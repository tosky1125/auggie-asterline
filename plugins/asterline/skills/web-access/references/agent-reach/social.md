...
> **前置条件**: 运营方须预先提供 `xiaohongshu-cli`；然后可运行 `xhs login`（自动从浏览器提取 Cookie）。本技能不安装依赖。
>
...
>
> **POST 操作风险**: 发帖(post)、评论(comment)、点赞(like) 等写操作在 v0.6.x 可能因签名问题返回 406。需要旧版时由运营方按策略提供 v0.3.5。
...
# 如果 search 返回 404，请运营方检查 twitter-cli 版本
...
> **前置条件**: 运营方须预先提供 `twitter-cli` v0.8.5+；本技能不安装或升级依赖。
>
...
>
> **search 可能失效**: Twitter 频繁修改 GraphQL API，search 命令可能随时返回 404。如遇到，请运营方检查版本；如果仍不行，说明上游还没跟上 Twitter 的改动，用 `twitter feed` 替代。
>
...
> **前置条件**: 运营方须预先提供 `rdt-cli` v0.4.2+；本技能不安装依赖。无需登录即可搜索和阅读。
