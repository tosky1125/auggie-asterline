...
## 하네스 규칙 (Auggie에게 강제되는 지침)

이 규칙은 Auggie가 즉흥 판단으로 엇나가지 못하게 하기 위한 **고삐**다. 위반 시 이전 test.md 세션처럼 "chrome 200에서 break → safari 미시도 → Playwright 미설치라 포기" 식의 오판이 재현된다.

**R1 — 일반 웹 URL 차단/403/402 감지 시**:
1. Auggie fetch, 즉흥 curl, 수동 헤더 조합 **시도 금지**
2. 즉시 다음을 실행:
   ```bash
...
단, R7 조건(WAF 조기 감지)이 성립하면 engine 격자는 계속 돌되, Auggie가 **병렬로** MCP 정찰 루트를 시도할 수 있다. 빠른 쪽이 이긴다.
...
세 조건 모두 참일 때 Auggie는 **병렬 경로**를 시작한다:

**"병렬"의 실행 의미** (Auggie 도구 호출이 순차이므로 명확화):
- engine은 `run_in_background=true`로 Bash 툴에서 띄워둔다 — 격자는 그대로 돌되 블로킹하지 않음
- Auggie는 그 사이 foreground에서 MCP Playwright 정찰 루트를 진행
- engine이 먼저 성공해도 좋고, MCP 정찰로 얻은 API가 먼저 성공해도 좋음. 빠른 쪽 결과 채택

**MCP 정찰 루트**:
1. 현재 Auggie 세션에 실제 제공된 브라우저 navigate 기능으로 대상 페이지 로드
2. 제공된 network-request 기능이 있으면 XHR/fetch 호출 목록을 수집하고 `/api/`·`/graphql`·`\.json` 필터로 내부 엔드포인트 식별
...
| `needs_js_exec` only | 현재 Auggie 세션의 브라우저 커넥터(제공된 경우만) | Cloudflare 기본 방어 등 |
...
`fetch_chain`의 `needs_js_exec only` 케이스는 현재 Auggie 세션에 브라우저 커넥터가 실제로 제공되는 경우에만 세션에서 처리한다. subprocess 경로는 없다. 즉:
1. `result.summary`에 Auggie browser connector가 필요하다고 표시되면
2. 사용 가능한 커넥터의 navigate → wait → snapshot 흐름으로 처리하고, 없으면 지원되지 않음을 보고한다
...
`python3 -c "import curl_cffi, bs4, yaml"`로 Tier 1 의존성을 확인한다. Local Chrome 경로는 `node -e "require('playwright'); require('playwright-extra'); require('puppeteer-extra-plugin-stealth')"`가 성공해야 한다. 어느 쪽이든 없으면 해당 경로를 중단하고 운영자에게 요청하며 Asterline은 설치를 실행하지 않는다.
...
이 섹션은 **참조 파일 선택 가이드**다. 문제가 생겼을 때 어떤 `references/*.md`를 열어야 할지 결정하는 기준으로 쓴다. Auggie는 필요할 때만 해당 파일을 `Read`하고, 선제적으로 전부 읽지 않는다.
...
| [`playwright.md`](playwright.md) | engine이 Playwright fallback으로 넘어가는데 Auggie connector/Local Chrome 중 어디로 갈지 확인 필요할 때 | Approach 1 (제공된 Auggie 브라우저 커넥터), Approach 2 (Local Node + `channel:'chrome'` + stealth), 템플릿 파라미터 규격 |
