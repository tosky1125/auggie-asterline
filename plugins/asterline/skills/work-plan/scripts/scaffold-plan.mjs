#!/usr/bin/env node
// scaffold-plan.mjs - generate the work-plan draft + plan skeleton deterministically.
//
...
// own writes to resolve under .asterline/ (it does not, and cannot, contain other Bash
// commands; it only guarantees the mandated generator never escapes .asterline). Mirrors
// the upstream Prometheus path-policy contract.
...
export function resolveSafeOmoPath(cwd, relPath) {
	...
	if (!/(^|[/\\])\.asterline([/\\]|$)/i.test(rel)) {
		throw new Error(`refused: work-plan may only write under .asterline/: ${relPath}`);
	}
	if (!resolved.toLowerCase().endsWith(".md")) {
		throw new Error(`refused: work-plan may only write .md files: ${relPath}`);
	}
	return resolved;
}
...
- Evidence: <attemptDir>/task-<N>-${slug}.<ext> (attemptDir = the current work-loop attempt directory; outside work-loop use .asterline/evidence/)
...
// Resume-safe write: plain re-run on an existing work-plan artifact is a no-op
...
export async function writeGuarded(cwd, relPath, content, { reset = false, force = false } = {}) {
	...
	if (existing && existing.trim() !== "") {
		if (!reset) {
			...
			throw new Error(`refused: ${relPath} exists and is not a work-plan artifact (pass --reset to overwrite)`);
		}
