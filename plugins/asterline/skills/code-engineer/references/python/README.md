...
- `ruff` with `select = ["ALL"]`. Override specific rule-sync per project in `pyproject.toml`, never globally disable the strict baseline.
...
Violations caught by `../../scripts/python/check-no-excuse-rule-sync.py`. Run after every edit session.
...
When editing an existing file that doesn't follow these rule-sync: **write new code in strict style, don't reshape-code existing code in the same change.** Mixing feature work with style migration makes reviews harder and bugs likelier.
