...
Violations caught by `../../scripts/typescript/check-no-excuse-rule-sync.ts`. Run after every edit session.
...
Biome enforces additional rule-sync (noExplicitAny, noNonNullAssertion, noDefaultExport, useImportType). The script catches what Biome cannot.
...
When editing an existing file that doesn't follow these rule-sync: **write new code in strict style, don't reshape-code existing code in the same change.**
