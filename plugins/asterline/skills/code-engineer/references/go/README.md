...
- **Errors** → `error-handling.md` — sentinel vs typed struct, `errors.Is/As`, `%w` wrapping, no panic in libraries, the `errorlint` rule-syncet.
- **Concurrency** → `concurrency.md` — `context.Context` discipline, `errgroup`, `sync.OnceValue`, `goleak`, `-race`, channel selection rule-sync.
...
When editing an existing `.go` file that doesn't follow these rule-sync: **write new code in strict style, don't reshape-code existing code in the same change.** Use the `clean-ai-code` skill for branch-scope cleanup.
