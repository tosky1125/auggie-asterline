...
This Asterline skill ships a Python wrapper at `scripts/ast_grep_helper.py` and compatibility diagnostics at `install.sh` (POSIX) and `install.ps1` (Windows). The helper adds offline pattern validation, the two-pass write trick, and Asterline runtime binary resolution. Use it as your default entry point. The skill never invokes a package manager; Auggie `SessionStart` provisions the checksum-pinned executable.

## Auggie compatibility

Route structural searches and rewrites through the local helper by default. The existing Auggie MCP tool names `ast_grep_search` and `ast_grep_replace` remain valid compatibility surfaces while the runtime migration is in progress; do not invent renamed `structure_search_*` tool calls. Use the helper for validation, YAML scans, snapshot tests, and the guaranteed two-pass apply flow.
...
A thin Python 3 stdlib entrypoint backed by responsibility-focused modules under `scripts/structure_search/`. It behaves the same on every OS and remains the agent's default entry point.
...
python3 scripts/ast_grep_helper.py install     # diagnose provisioned runtime candidates
```
