...
# Smoke test for Asterline structure-search on Windows (PowerShell 5.1+).
...
try {
    ...
    if ($fm -notmatch '(?m)^name:\s*structure-search\s*$') { Fail 'frontmatter missing name: structure-search' }
