...
# Given ASTERLINE_AST_GREP_SG_PATH points at a fake executable.
...
RUNTIME_SLUG="$(asterline_runtime_slug)"
RUNTIME_BIN="sg"
case "$RUNTIME_SLUG" in
  win32-*) RUNTIME_BIN="sg.exe" ;;
esac
FAKE_RUNTIME_SG="$RUNTIME_HOME/.asterline/runtime/ast-grep/$RUNTIME_SLUG/$RUNTIME_BIN"
fake_sg "$FAKE_RUNTIME_SG"
HOME="$RUNTIME_HOME" ASTERLINE_HOME= ASTERLINE_AST_GREP_SG_PATH= $HELPER doctor > "$OUTPUT_DIR/asterline-runtime.out" 2>&1
grep -Fq "ast-grep binary: $FAKE_RUNTIME_SG" "$OUTPUT_DIR/asterline-runtime.out" || fail "Asterline HOME runtime was not resolved: $(cat "$OUTPUT_DIR/asterline-runtime.out")"
pass "Asterline HOME runtime resolves before standalone fallback"
...
fm = src[4:end]
assert re.search(r"^name:\s*structure-search\s*$", fm, re.M), "frontmatter missing name: structure-search"
